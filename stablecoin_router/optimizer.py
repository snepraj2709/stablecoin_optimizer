...
Original functionality preserved (PuLP single-transaction optimization).
Enhancements:
 - Candidate route generation from eligible venues (re-uses your cost/time/risk logic)
 - Top-K candidate pruning utility
 - Optional use of top-K pruning inside optimize() (non-breaking; disabled by default)
 - New method optimize_batch_mip(): batched integer routing using OR-Tools CP-SAT
   (each transaction picks exactly one route from its top-K candidates).
 - Additional logging and solver metadata.
"""
...
# Conditional import of OR-Tools for batched MIP solver
try:
...
class UnifiedOptimizer:
    """
    Multi-objective route optimizer (PuLP)
    Enhancements:
     - top_k pruning of candidate venues
     - candidate route generation helper
     - optional batched OR-Tools CP-SAT solver via optimize_batch_mip()
    """

    def __init__(self, catalog: VenueCatalog, top_k: Optional[int] = None):
        self.catalog = catalog
        self.optimization_count = 0
        self.top_k = top_k

    # Candidate generation & pruning
    def _generate_candidate_routes(self, normalized_tx: NormalizedTransaction, eligible_venues: List[Venue]) -> List[Dict[str, Any]]:
        """
        Convert eligible Venue objects to candidate route dicts using existing cost/time/risk logic.
        Returns list of route dicts:
          {
            'venue_id', 'venue_name', 'est_cost_bps', 'est_time_sec', 'risk_score', 'asset'
          }
        """
        amount = normalized_tx.amount_usd
        candidates = []
        for v in eligible_venues:
            base_cost_rate = v.base_fee_bps / 10000  # bps -> rate
            gas_cost = v.gas_cost_usd
            avg_slippage_rate = v.slippage_coefficient * (amount / (v.available_liquidity_usd or 1.0))
            cost_per_dollar = base_cost_rate + avg_slippage_rate
            # express est_cost_bps as bps for parity with other code
            # variable component in bps: cost_per_dollar * 10000
            est_cost_bps = cost_per_dollar * 10000.0
            total_time = v.avg_execution_time_sec + v.avg_settlement_time_sec
            risk_score = (1 - v.reliability_score)  # keep 0..1 risk scale here
            asset = getattr(v, "settlement_asset", v.venue_id)  # best-effort asset tag
            candidates.append({
                "route_id": v.venue_id,
                "venue_name": v.venue_name,
                "est_cost_bps": est_cost_bps,
                "est_time_sec": total_time,
    ...
    def prune_top_k_routes(self, tx_candidates: List[Dict[str, Any]], top_k: int, score_weights: Dict[str, float] = None) -> List[Dict[str, Any]]:
        """
        Keep top_k candidates by pre-score.
        score_weights expects dict {'alpha','beta','gamma'} to weigh cost/time/risk.
        Pre-score construction tries to make components comparable.
        """
        if not tx_candidates:
            return []
        if score_weights is None:
            score_weights = {"alpha": 0.6, "beta": 0.3, "gamma": 0.1}

        max_time = max(r["est_time_sec"] for r in tx_candidates) or 1.0
        # cost in bps might vary widely; risk 0..1
        scored = []
        for r in tx_candidates:
            time_norm = r["est_time_sec"] / max_time
            # scale time_norm and risk to comparable magnitudes with cost_bps
            pre_score = (score_weights["alpha"] * r["est_cost_bps"]
                         + score_weights["beta"] * time_norm * 1000.0
                         + score_weights["gamma"] * r["risk_score"] * 1000.0)
            scored.append((pre_score, r))
        scored.sort(key=lambda x: x[0])
        pruned = [r for (_, r) in scored[:top_k]]
        logger.debug(f"Pruned {len(tx_candidates)} -> {len(pruned)} candidates (top_k={top_k})")
        return pruned

    # -----------------------------
    # Existing PuLP-based optimizer
    # -----------------------------
    def optimize(self, normalized_tx: NormalizedTransaction) -> OptimizationResult:
        """
        Optimize routing for a single normalized transaction (PuLP LP).
        This preserves the original behavior, but if self.top_k is set, the eligible venues
        will be pruned to top_k prior to building the LP model (non-breaking: default top_k=None).
        """
        ...
        # Candidate generation (for optional top-K pruning and for the MIP batch solver)
        candidates = self._generate_candidate_routes(normalized_tx, eligible_venues)

        # Optionally prune to top-K candidates (non-breaking: only active if self.top_k is an int)
        used_venues = eligible_venues
        if self.top_k is not None and isinstance(self.top_k, int) and self.top_k > 0:
            pruned = self.prune_top_k_routes(candidates, self.top_k)
            # Map pruned back to Venue objects
            pruned_venue_ids = {p['route_id'] for p in pruned}
            used_venues = [v for v in eligible_venues if v.venue_id in pruned_venue_ids]
            logger.info(f"Applied top_k pruning: {len(eligible_venues)} -> {len(used_venues)} venues")
        else:
            logger.debug("Top-K pruning disabled (top_k=None)")

        # --- Original PuLP LP model follows, unchanged except we use used_venues instead of eligible_venues ---
        ...
        model += (
            lpSum([route_amount[v.venue_id] for v in used_venues]) == amount,
            "Fill_Complete_Order"
        )
        ...
        if normalized_tx.max_routes:
            model += (
                lpSum([route_used[v.venue_id] for v in used_venues]) <= normalized_tx.max_routes,
        ...
        if status in ["Optimal", "Feasible"]:
            return self._extract_result(
                model, normalized_tx, used_venues,
    ...
    def _extract_result(
        ...
        """Extract optimization results from solved model (unchanged logic)"""
    ...
    # -----------------------------
    # Batch optimization (original loop)
    # -----------------------------
    def optimize_batch(
            self,
        ...
        """
        Optimize a batch of transactions sequentially via self.optimize(...)
         Args:
    ...
    # -----------------------------
    # New: Batched MIP (OR-Tools CP-SAT)
    # -----------------------------
    def optimize_batch_mip(
            self,
            transactions: List[NormalizedTransaction],
            liquidity_available: Optional[Dict[str, float]] = None,
            alpha: float = 1.0,
            beta: float = 1.0,
            gamma: float = 1.0,
            top_k: int = 4,
        ...
        """
        Batched integer optimizer using OR-Tools CP-SAT.
        Each transaction picks exactly one route among its top_k candidates.
        This is an *alternative* batched solver—call explicitly when you want global optimization.

        Inputs:
         - transactions: list of NormalizedTransaction
         - liquidity_available: dict asset -> float (same units as tx.amount_usd). If provided,
             the model enforces sum(amount routed on asset) <= available.
         - alpha/beta/gamma: objective weights (will be multiplied against route features)
         - top_k: number of candidates to keep per tx
         - time_limit_sec: solver time limit

        Returns a dict:
         {
           "selected_routes": [ {tx_id, chosen_route (dict), tx_amount, obj_contrib_int} ],
        ...
        if not ORTOOLS_AVAILABLE:
            raise RuntimeError("optimize_batch_mip requires OR-Tools (ortools). Install ortools to use this function.")

        # Prepare candidates per tx
        all_candidates = []
        for tx in transactions:
            # Generate eligible venues as in optimize()
            ...
            if tx.region:
                eligible_venues = [v for v in eligible_venues if tx.region in v.supported_regions]
            if tx.is_cross_border:
                eligible_venues = [v for v in eligible_venues if v.supports_cross_border]
            candidates = self._generate_candidate_routes(tx, eligible_venues)
            # SLA filter
            if tx.max_settlement_time_sec:
                candidates = [c for c in candidates if c["est_time_sec"] <= tx.max_settlement_time_sec]
            if not candidates:
                return {
                    "selected_routes": [],
                    "objective_value": None,
                    "feasible": False,
                    "message": f"No feasible candidates for tx {tx.transfer_id} after SLA/regional filtering."
                }
            pruned = self.prune_top_k_routes(candidates, top_k)
            if not pruned:
                return {
                    "selected_routes": [],
                    "objective_value": None,
                    "feasible": False,
                    "message": f"No feasible candidates for tx {tx.transfer_id} after pruning."
                }
            all_candidates.append(pruned)

        # Build CP-SAT model
        model = cp_model.CpModel()
        x_vars = []  # x_vars[t_idx][r_idx] boolean
        for t_idx, cands in enumerate(all_candidates):
            row = []
            for r_idx, _ in enumerate(cands):
                row.append(model.NewBoolVar(f"x_t{t_idx}_r{r_idx}"))
            x_vars.append(row)
            # Each tx picks exactly one route
            model.Add(sum(row) == 1)

        # Liquidity constraints
        SCALE = 10**6
        if liquidity_available:
            # Build exprs per asset
            asset_pairs = {}  # asset -> list of (var, coeff_int)
            for t_idx, tx in enumerate(transactions):
                amt = tx.amount_usd
                amt_int = int(round(amt * SCALE))
                for r_idx, route in enumerate(all_candidates[t_idx]):
                    asset = route.get("asset", route.get("route_id"))
                    asset_pairs.setdefault(asset, []).append((x_vars[t_idx][r_idx], amt_int))
            for asset, var_coeffs in asset_pairs.items():
                available = liquidity_available.get(asset, 0.0)
                avail_int = int(round(available * SCALE))
                model.Add(sum(var * coeff for var, coeff in var_coeffs) <= avail_int)

        # Objective: weighted sum of route features
        objective_terms = []
        for t_idx, tx in enumerate(transactions):
            for r_idx, route in enumerate(all_candidates[t_idx]):
                raw_score = (alpha * route["est_cost_bps"]
                             + beta * (route["est_time_sec"] / 1.0)
                             + gamma * (route["risk_score"] * 100.0))
                coeff = int(round(raw_score * SCALE))
                objective_terms.append(coeff * x_vars[t_idx][r_idx])
        ...
        if feasible:
            ...
            obj_value = float(obj_int) / SCALE
            for t_idx, tx in enumerate(transactions):
                chosen_idx = None
                for r_idx, _ in enumerate(all_candidates[t_idx]):
                    if solver.Value(x_vars[t_idx][r_idx]) == 1:
                        chosen_idx = r_idx
                        break
                chosen_route = all_candidates[t_idx][chosen_idx] if chosen_idx is not None else None
                selected.append({
                    "tx_id": tx.transfer_id,
                    "chosen_route": chosen_route,
                    "tx_amount": tx.amount_usd
                })

        solver_info = {
            "status": solver.StatusName(status),
            "wall_time_sec": solver.WallTime(),
            "best_objective_int": solver.ObjectiveValue() if feasible else None
        }
        return {
